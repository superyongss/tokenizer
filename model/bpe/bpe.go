...
func (bb *BpeBuilder) Box(box *packr.Box) {
	bb.box = box
}
...
func (bb *BpeBuilder) Build() (*BPE, error) {
	...
	if bb.config.files != nil {
		if bb.box != nil {
			vocab, merges, err = bpe.ReadBoxFiles(bb.box, bb.config.files.vocab, bb.config.files.merges)
			if err != nil {
				return nil, err
			}
		} else {
...
func (b *BPE) ReadBoxFiles(box *packr.Box, vocabF string, mergesF string) (*model.Vocab, *Merges, error) {
	var err error

	// read json file
	vocabBytes, err := box.Find(vocabF)
	if err != nil {
		return nil, nil, err
	}
	...
	mergeBytes, err := box.Find(mergesF)
	if err != nil {
		return nil, nil, err
	}

	s := bufio.NewScanner(bytes.NewReader(mergeBytes))
